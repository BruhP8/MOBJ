...
namespace Netlist {
  ...
  inline Box&  Box::inflate         ( int dx1, int dy1, int dx2, int dy2 ) {
      x1_ -= dx1; y1_ -= dy1; x2_ += dx2; y2_ += dy2; return *this; }  
  inline Box   Box::getIntersection ( const Box& other ) {
  ...
  inline bool  Box::intersection ( const Box& other ) const
  {
